...
$text = file_get_contents($url);
	...
	if(strpos($text,'true')) {
		...
		$sqlacharya = "UPDATE acharya SET adate=?, aguru=? WHERE acharyaid='$id'";
		$q = $db->prepare($sqlacharya);
		$q->execute(array($adate,$aguru));
		
		$sqlailacharya = "UPDATE ailacharya SET ailacharyaname=?, ailacharyadate=?, ailacharyaguru=? WHERE ailacharyaid='$id'";
		$q = $db->prepare($sqlailacharya);
		$q->execute(array($ailacharyaname,$ailacharyadate,$ailacharyaguru));
		
		$sqlupadhyay = "UPDATE upadhyay SET upadhyayname=?, upadhyaydate=?, upadhyayguru=? WHERE upadhyayid='$id'";
		$q = $db->prepare($sqlupadhyay);
		$q->execute(array($upadhyayname,$upadhyaydate,$upadhyayguru));
		
		$sqlmuni = "UPDATE muni SET muniname=?, munidate=?, muniguru=? WHERE muniid='$id'";
		$q = $db->prepare($sqlmuni);
		$q->execute(array($muniname,$munidate,$muniguru));
		
		$sqlailak = "UPDATE ailak SET ailakname=?, ailakdate=?, ailakguru=? WHERE ailakid='$id'";
		$q = $db->prepare($sqlailak);
		$q->execute(array($ailakname,$ailakdate,$ailakguru));
		
		$sqlkshullak = "UPDATE kshullak SET kname=?, kdate=?, kguru=? WHERE kid='$id'";
		$q = $db->prepare($sqlkshullak);
		$q->execute(array($kname,$kdate,$kguru));
		
		$sqlaryika = "UPDATE aryika SET aryikadate=?, aryikaguru=? WHERE aryikaid='$id'";
		$q = $db->prepare($sqlaryika);
		$q->execute(array($aryikadate,$aryikaguru));
